    ...
    <div class="course-card-body">
      ...
      <div v-if="enrollment" style="margin-top:12px">
        <div style="display:flex;justify-content:space-between;margin-bottom:6px">
          <span class="text-sm text-muted">
            {{ isOnsite ? $t('courseCard.attendance') : $t('courseCard.progress') }}
          </span>
      ...
      <div class="course-card-meta">
        <div class="course-meta-item text-sm text-muted">
          <strong style="color:var(--lf-black)">{{ course.lesson_count ?? course.lessons?.length ?? 0 }}</strong>
          {{ $t('courseCard.lessons', {n: ''}).replace('0', '') }}
        </div>
        <div style="display:flex;gap:6px;align-items:center">
          <span v-if="examResult" class="badge badge-green">
            {{ $t('courseCard.examResult', {score: examResult.score}) }}
          </span>
          <span v-else-if="course.exam && user.role ==='student'" class="badge badge-orange">
           {{ $t('courseCard.examPending') }}
          </span>
          <span v-else-if="course.exam && user.role ==='admin'" class="badge badge-orange">
            {{ $t('courseCard.hasExam') }}
          </span>
          <span v-if="hasCert" class="badge badge-green">🎓 {{ $t('courseCard.certified') }}</span>
          <span v-if="!course.exam && !enrollment" class="badge badge-gray">
            {{ $t('courseCard.noExam') }}
          </span>
        </div>
      </div>
    ...
    >
      <div class="status-bar-top">
        <span class="status-badge" :class="course.status">
          {{
            course.status === 'published' ? '✓ ' + $t('courseCard.status.published') : '✎ ' + $t('courseCard.status.draft')
          }}
        </span>
        <button
            class="status-btn"
        ...
        >
          {{
            toggling ? '…' : (course.status === 'published' ? $t('courseCard.actions.unpublish') : $t('courseCard.actions.publish'))
          }}
        </button>
      </div>
      <div class="status-bar-meta">
        <span v-if="course.status === 'published' && course.published_at">
          {{ $t('courseCard.status.liveSince', {date: formatDate(course.published_at)}) }}
        </span>
        <span v-else>
          {{
            $t('courseCard.status.lessonsPublished', {
              published: course.published_lesson_count ?? 0,
...
.course-card.has-status-bar {
  border-radius: 8px;
}

.course-card.has-status-bar .course-card-body {
  border-radius: 0;
}
...
.btn-publish:hover:not(:disabled) {
  ...
  border-color: var(--lf-orange-dark);
}
