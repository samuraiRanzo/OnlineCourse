<template>
  <div class="page-content">
    ...
    <div class="page-actions-bar">
      <div class="page-actions-title display">Students</div>
    ...
    <div class="lf-card" style="padding:0;overflow:hidden">
      <EmptyState
          ...
          message="Add your first student above."
          style="padding:40px"
      />

      <!--
        table-scroll-wrap is the key fix for mobile:
          overflow-x: auto       → enables horizontal scroll when content overflows
          -webkit-overflow-scrolling: touch → smooth momentum scroll on iOS
          max-width: 100%        → prevents the wrapper itself from expanding past the card
        Without these three together, the table bleeds off-screen on narrow viewports.
      -->
      <div v-else class="table-scroll-wrap">
        <table class="lf-table">
          ...
          <tr>
            ...
            <!-- hidden on mobile — email visible in expanded row -->
            <th class="col-email">Email</th>
            <!-- hidden on mobile to save space -->
            <th class="col-enrolled">Enrolled In</th>
            <th class="col-certs">Certs</th>
          ...
          <tr v-for="s in students" :key="s.id">
            ...
            <td>
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div>
                  ...
                  <!-- Email shown here on mobile (col-email is hidden) -->
                  <div class="mobile-only text-muted" style="font-size:11px;margin-top:2px">
                    {{ s.email }}
                  </div>
                </div>
              </div>
            ...
            <td class="col-email text-muted">{{ s.email }}</td>

            <!-- Enrolled In — hidden on mobile -->
            <td class="col-enrolled">
              <span v-if="!enrolledCourses(s.id).length" class="text-muted text-sm">None</span>
              <span
                  v-for="c in enrolledCourses(s.id)"
                  :key="c.id"
  ...
  <BaseModal v-model="showAdd" title="Add New Student">
    <FormGroup label="Full Name">
      <input v-model="form.name" class="form-control" placeholder="Jane Doe"/>
    </FormGroup>
...
function enrollAvailable(studentId) {
  return enrolledCourses(studentId).length < coursesStore.courses.length
}
...
function openEnroll(student) {
  ...
  enrollCourseId.value = availableCourses.value[0]?.id ?? ''
...
onMounted(async () => {
  await Promise.all([
    ...
    coursesStore.fetchEnrollments().then(e => {
      enrollments.value = e
    }),
  ])
})
...
/* ── TABLE SCROLL WRAPPER ────────────────────────────────────────────────────
   ...
   The wrapper must be a direct block container with:
     overflow-x: auto          → scrolls the table horizontally when it overflows
     -webkit-overflow-scrolling: touch → smooth momentum scroll on iOS Safari
     max-width: 100%           → prevents wrapper itself from being wider than its parent

   Without max-width: 100% the lf-card can still expand and push content off-screen.
   Without overflow-x: auto the table just bleeds past the card edge.
   ──────────────────────────────────────────────────────────────────────────── */
.table-scroll-wrap {
  overflow-x: auto;
  -webkit-overflow-scrolling: touch;
  max-width: 100%;
  /* Show a subtle scroll indicator on the right edge on mobile */
...
.lf-table {
  ...
  /* Do NOT set width: 100% or max-width here — let the table size to its content
     so the scroll wrapper can actually scroll. min-width prevents it collapsing
     too small on narrow viewports. */
  min-width: 600px;
}
...
.lf-table th {
  ...
  white-space: nowrap;
  background: var(--lf-white); /* sticky header background */
}
...
/* ── Responsive column visibility ────────────────────────────────────────────
   On screens narrower than 768px:
     - Hide Email column (shown inline under name instead)
     - Hide Enrolled In column (less critical on mobile)
     - Hide Certs column (less critical on mobile)
   This dramatically reduces the table width and often eliminates the need
   to scroll at all on mid-size mobile screens.
   ──────────────────────────────────────────────────────────────────────────── */
.mobile-only {
  display: none;
}
...
@media (max-width: 768px) {
  ...
  /* Allow min-width to shrink more aggressively */
  .lf-table {
    min-width: 340px;
  }
  ...
  /* Page actions bar stacks on very small screens */
  .page-actions-bar {
...
@media (max-width: 480px) {
  /* On very small phones, only show name + actions */
  .col-type {
    display: none;
  }
