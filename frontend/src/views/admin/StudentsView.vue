<template>
  <div>
        ...
        <div v-else class="table-wrap">
          <table class="lf-table">
            ...
            <tr v-for="s in students" :key="s.id">
              ...
              <td><span :class="['type-pill', `type-${s.student_type}`]">{{
                  s.student_type === 'online' ? '💻 Online' : '🏫 On-site'
                }}</span></td>
              <td class="text-muted">{{ s.email }}</td>
              <td>
                <span v-if="!enrolledCourses(s.id).length" class="text-muted text-sm">None</span>
                <span v-for="c in enrolledCourses(s.id)" :key="c.id" class="badge badge-gray" style="margin-right:4px">{{
                    c.course_title
                  }}</span>
              </td>
              <td>
              ...
              <td>
                <div style="display:flex;gap:6px; justify-content: end">
                  <button v-if="enrollAvailable(s.id)" class="btn btn-ghost btn-sm" @click="openEnroll(s)">Enroll
                  </button>
    ...
    <BaseModal v-model="showAdd" title="Add New Student">
      ...
      <FormGroup label="Email"><input v-model="form.email" type="email" class="form-control"
                                      placeholder="jane@example.com"/></FormGroup>
...
// FIX: computed so the template uses it reactively without calling it as a function
...
function openEnroll(student) {
  ...
  // Set default after selectedStudent is set so the computed has the right value
  enrollCourseId.value = availableCourses.value[0]?.id ?? ''
}

function enrollAvailable(studentId) {
  const enrolled = enrolledCourses(studentId).map(e => e.course)
  return enrolled.length < coursesStore.courses.length
}
