  ...
  <div>
    ...
    <div class="page-content">
      ...
      <div class="filter-tabs">
        <button
          v-for="tab in ['all', 'published', 'draft']" :key="tab"
          class="filter-tab"
          :class="{ active: statusFilter === tab }"
          @click="statusFilter = tab"
        >
          {{ tab === 'all' ? 'All' : tab === 'published' ? '✓ Published' : '✎ Drafts' }}
          <span class="filter-count">{{ tabCount(tab) }}</span>
        </button>
      </div>

      <EmptyState v-if="!filteredCourses.length && !loading" icon="📚" title="No courses yet" message="Create your first course." />
    ...
    <BaseModal v-model="showCreate" title="Create New Course">
      ...
      <div style="background:var(--lf-orange-light);border:1.5px solid var(--lf-orange);border-radius:6px;padding:12px;font-size:13px;color:var(--lf-orange-dark)">
...
const toggling      = ref(null)   // courseId currently being toggled
...
.filter-tab  {
  ...
  background: transparent; border: 1.5px solid var(--lf-gray-200);
  color: var(--lf-gray-600); cursor: pointer; transition: all .15s;
  display: flex; align-items: center; gap: 6px;
}
.filter-tab:hover  { border-color: var(--lf-black); color: var(--lf-black); }
.filter-tab.active { background: var(--lf-black); color: #fff; border-color: var(--lf-black); }
.filter-count { font-size: 11px; padding: 1px 6px; border-radius: 10px; background: var(--lf-gray-200); color: var(--lf-gray-600); }
...
.btn { display: inline-flex; align-items: center; padding: 6px 14px; border: none; border-radius: var(--lf-radius); font-family: var(--lf-font-body); font-size: 12px; font-weight: 600; cursor: pointer; transition: all .15s; }
